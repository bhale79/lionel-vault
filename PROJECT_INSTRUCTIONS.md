...
8. **Cache versions must always bump together.** Both `_CACHE_VER` in app.js AND `CACHE_NAME` in sw.js must be incremented on every deploy. Missing one means users get stale code.
...
1. Read SESSION_77_SUMMARY.md (or latest) and ARCHITECTURE_14.md for current state
