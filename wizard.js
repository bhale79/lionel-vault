...
function renderWizardStep() {
  ...
  if (s.type === 'itemCategory') {
    ...
    _qe1SaveBtn.onclick = function() {
      ...
      // Also disable Full Entry button to prevent both paths firing
      var _feBtn = _qe1SaveBtn.parentNode && _qe1SaveBtn.parentNode.querySelector('[data-qe1-full]');
      if (_feBtn) { _feBtn.disabled = true; _feBtn.style.opacity = '0.5'; }
      quickEntryAdd().catch(function(err) {
        wizard.data._qeSaving = false;
        wizard.data._wizSaveLock = false;
        _qe1SaveBtn.disabled = false;
        _qe1SaveBtn.textContent = '\u26a1 Save quick entry';
        if (_feBtn) { _feBtn.disabled = false; _feBtn.style.opacity = '1'; }
    ...
    _qe1FullBtn.setAttribute('data-qe1-full', '1');
