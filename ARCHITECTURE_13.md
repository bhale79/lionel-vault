# ARCHITECTURE_13.md — The Rail Roster

**Updated:** Session 76 (March 27, 2026)  
**Live:** therailroster.com  
**Repo:** github.com/bhale79/my-collection-app  
**Cache:** `_CACHE_VER = '47'` / `CACHE_NAME = 'mca-v52'`
...
| `app.js` | ~6,643 | Core: auth, state, data loading, SHEET_TABS config, buildPartnerMap, baseItemNum, suggestSets, item detail, For Sale/Sold/Want flows, dashboard, row deletion |
| `wizard.js` | ~8,065 | All wizard flows: collection, set (with My Sets write), want, sold, for sale, ephemera, IS, manual entry, custom tender pairing |
| `browse.js` | ~1,020 | Master list rendering, collection views (card + list), My Sets ownership display, ephemera tabs |
...
state = {
  ...
  personalData: {},        // keyed by "itemNum|variation|row" (migrating to inventoryId)
  forSaleData: {},         // keyed by "itemNum|variation"
  soldData: {},
  wantData: {},
  upgradeData: {},
  setData: [],             // 470 sets from Sets tab (21 columns A-U)
  mySetsData: {},          // keyed by "setNum|year|row" — owned sets from personal My Sets tab
...
- **`findPD(itemNum, variation)`** — finds personalData entry; has -P/-D fallback with exact-match verification
- **`findPDKey(itemNum, variation)`** — returns the state key for findPD
...
- Group deletes: sorted bottom-to-top to avoid row shift
- `_reloadAfterDelete()`: busts cache, reloads 1.5s later
...
| Data cache ver | `'47'` |
| SW cache name | `'mca-v52'` |
| Vault folder | "The Rail Roster - My Collection" |
| Config file | "rail-roster-config.json" |

---

## Pending: InventoryId Key Migration

**Current:** State keys use row numbers (e.g. `itemNum|variation|rowNum`). Row numbers shift on delete, causing stale references. `_reloadAfterDelete()` is a band-aid.

**Planned:** All state keys will use inventoryId instead. Row numbers kept on each record for Sheets API range calls only. Affects all 7 data types, all save/delete/lookup paths across app.js, wizard.js, browse.js. Brad approved as top priority for next session.
